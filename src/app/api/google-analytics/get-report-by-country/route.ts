...
import { fetchAnalyticsReportByCountry } from "@/utils/server/projects/analyticsAPI/google/fetchReport";
import { getOneProject } from "@/utils/server/projects/getOneProject";
import { getServerSession } from "next-auth";
import { cookies } from "next/headers";
import { NextRequest, NextResponse } from "next/server";

export interface GoogleAnalyticsReportByCountryRequestInterface {
    dateRange: {
        from: string,
        to: string,
    },
...
export async function POST(request: NextRequest) {
    try {
        ...
        const {
            dateRange,
        } = (await request.json()) as GoogleAnalyticsReportByCountryRequestInterface;
        ...
        const report = await fetchAnalyticsReportByCountry({
            auth,
